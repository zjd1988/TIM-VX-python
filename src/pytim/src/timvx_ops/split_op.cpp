...
namespace TimVX
{
    ...
    bool SplitCreator::parseAxisAttr(const json& op_info, SplitOpAttr& op_attr)
    {
        return parseValue<uint32_t>(op_info, m_op_name, "axis", op_attr.axis);
    }

    bool SplitCreator::parseSlicesAttr(const json& op_info, SplitOpAttr& op_attr)
    {
        return parseDynamicList<uint32_t>(op_info, m_op_name, "slices", op_attr.slices);
    }

    bool SplitCreator::parseOpAttr(const json& op_info, SplitOpAttr& op_attr)
    {
        return parseAxisAttr(op_info, op_attr) && parseSlicesAttr(op_info, op_attr);
    }

    Operation* SplitCreator::onCreate(std::shared_ptr<Graph>& graph, const json& op_info)
    {
        SplitOpAttr op_attr;
    ...
    REGISTER_OP_CREATOR(SplitCreator, Split);
